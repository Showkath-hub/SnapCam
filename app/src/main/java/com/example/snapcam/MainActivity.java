...
public class MainActivity extends AppCompatActivity {
    ImageButton capture, toggleFlash, flipCamera, Picbtn;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                requestPermissions(new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, 3098);
                return;
            }
        }
    ...
    public void takePicture(ImageCapture imageCapture) {
        final File file = new File(getExternalFilesDir(null), "image"+System.currentTimeMillis() + ".jpg");
...
//
                        ...
                        Log.e("surya",file.getAbsolutePath());
                        new Handler().postDelayed(new Runnable() {
                            ...
                            public void run() {
                                ...
                                UploadTask uploadTask = storageRef.child("images/"+file.getName()).putFile(Uri.fromFile(file));
                                ...
                                uploadTask.addOnProgressListener(taskSnapshot -> {
                                   ...
                                   progressBar.setProgress((int) progress);
                                        ...
                                        // Or, you can simply stop the animation: animationView.cancelAnimation();
                                ...
                                uploadTask.addOnSuccessListener(taskSnapshot -> {
                                    ...
                                    // Hide both the LottieAnimationView and ProgressBar when the upload is complete

                                    storageRef.child("images/"+file.getName()).getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                                        @Override
                                    ...
                                    // You can get the download URL if needed: taskSnapshot.getMetadata().getReference().getDownloadUrl()
                                    Toast.makeText(getApplicationContext(), "File uploaded successfully", Toast.LENGTH_SHORT).show();
                                    progressBar.setVisibility(View.GONE);

                                }).addOnFailureListener(exception -> {
                                    // File upload failed
                                    Log.e("UploadMsg", exception.getMessage());
    ...
    private void setFlashIcon(Camera camera) {
        if (camera.getCameraInfo().hasFlashUnit()) {
            if (camera.getCameraInfo().getTorchState().getValue() == 0) {
                ...
                toggleFlash.setImageResource(R.drawable.flash_off_24);
            } else {
